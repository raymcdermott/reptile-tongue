(ns repl-ui.views
  (:require [re-frame.core :as re-frame]
            [re-com.core :refer [h-box v-box box button gap line scroller border label input-text md-circle-icon-button
                                 md-icon-button input-textarea modal-panel h-split v-split title flex-child-style p]]
  ...
  (try
    (reinsert-strings
      (first (edn/read-string
               (specter/transform [(specter/walker string?)]
                                  gs/unescapeEntities
                                  (:pretty result)))))
    (catch js/Error e
      (do                                                   ; TODO Explain fail
        (println "Oops: " e)
        (:original-form result)))))                         ;; Bug ... why does the read fail?

(defn format-result
  [result]
  (let [eval-result  (:eval-result result)
        smart-val    (prettify result)                      ; Hack
  ...
  (fn []
    [border
     ...
     :child [v-box
             ...
             :children
             [[title :label "Add a dependency" :level :level2]
              [v-box
               :gap "10px"
               ...
               :children [[label :label "Name"]
                          [input-text
                           :model (:name @lib-data)
                           ; TODO - conform to a Maven spec
                           :on-change #(swap! lib-data assoc :name %)]
                          [label :label "Version"]
                          [input-text
                           :model (:version @lib-data)
                           ; TODO - conform to a Maven version
...
(defn edit-panel
  ...
  (let [show-add-lib? (reagent/atom false)
        lib-data      (reagent/atom {:name "?" :version "?"})
        add-lib-event (fn []
...
(defn main-panels
  ...
  [v-box :height "625px" :children
   [[v-split :splitter-size "2px" :initial-split "35%"
     :panel-1 [eval-panel]
