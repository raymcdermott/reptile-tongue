...
(defn heading []
  (let [name (re-frame/subscribe [::subs/name])]
    [title :label (str "REPtiLe by " @name)
     :level :level4
...
(defn format-result
  [result]
  (list [label :label (:form result)]
        [label :label (str "=> " (:val result)) :on-click #(js/alert result)]
        [gap :size "10px"]))

(defn eval-panel
  []
  (#_ [box :size "auto" :child
    [:div (list [:span {:style "font-weight:bold;color:red"} "("]
                [:span {:style "font-weight:bold;color:blue"} "+"]
                " "
                [:span {:style "color:cyan"} "1"]
                " "
                [:span {:style "color:cyan"} "2"]
